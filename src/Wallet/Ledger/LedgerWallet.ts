...
import { getAppAxc, getAppEth, getEthAddressKeyFromAccountKey, getLedgerConfigAxc } from '@/Wallet/Ledger/utils';
...
export class LedgerWallet extends PublicMnemonicWallet {
    ...
    static async setTransport(transport: Transport) {
        ...
        // Update the config
        const config = await getLedgerConfigAxc(transport);
    ...
    static async fromTransport(transport: Transport, accountIndex = 0) {
        ...
        const pubAxc = await LedgerWallet.getExtendedPublicKeyAxcAccount(transport, accountIndex);
        const pubEth = await LedgerWallet.getExtendedPublicKeyEthAddress(transport, accountIndex);

        let config = await getLedgerConfigAxc(transport);
     ...
     */
    static async getExtendedPublicKeyAxcAccount(transport: Transport, accountIndex = 0): Promise<string> {
     ...
     * @param transport
     */
    static async getAxcConfig(transport: Transport) {
    ...
    >(unsignedTx: UnsignedTx, paths: string[], chainId: ChainIdType): Promise<SignedTx> {
        ...
        const accountPath =
            chainId === 'C'
                ...
                : bippath.fromString(getAccountPathAxia(this.accountIndex));
        let txbuff = unsignedTx.toBuffer();
        let changePath = this.getChangeBipPath(unsignedTx, chainId);
        //@ts-ignore
        let ledgerSignedTx = await appAxc.signTransaction(accountPath, bip32Paths, txbuff, changePath);
    ...
    async signHash(accountPath: any, bip32Paths: any, hash: Buffer): Promise<Map<string, Buffer>> {
        ...
        const appAxc = getAppAxc(LedgerWallet.transport);
        //@ts-ignore
        return await appAxc.signHash(accountPath, bip32Paths, hash);
    }
    ...
    >(unsignedTx: UnsignedTx, paths: string[], chainId: ChainIdType): Promise<SignedTx> {
        ...
        //@ts-ignore
        let sigMap = await appAxc.signHash(accountPath, bip32Paths, msg);
