import { activeNetwork, axChain, web3 } from '@/Network/network';
...
export async function getBaseFee(): Promise<BN> {
    const rawHex = (await axChain.getBaseFee()).substring(2);
...
export async function getMaxPriorityFee(): Promise<BN> {
    const rawHex = (await axChain.getMaxPriorityFeePerGas()).substring(2);
    return new BN(rawHex, 'hex');
}
