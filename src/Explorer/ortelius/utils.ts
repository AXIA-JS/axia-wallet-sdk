 ...
 * @param txs
 */
export function filterDuplicateOrtelius(txs: OrteliusAxiaTx[]) {
    let txsIds: string[] = [];
    let filtered: OrteliusAxiaTx[] = [];
 ...
 */
export function findSourceChain(tx: OrteliusAxiaTx): string {
 ...
 * @param tx Tx data from the explorer.
 */
export function findDestinationChain(tx: OrteliusAxiaTx): string {
    let baseChain = tx.chainID;
    let outs = tx.outputs || [];
...
export function getStakeAmount(tx: OrteliusAxiaTx): BN {
    let outs = tx.outputs || [];
