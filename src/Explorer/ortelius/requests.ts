...
import { OrteliusAxiaTx, OrteliusEvmTx } from '@/Explorer';
 ...
 * @param txID
 */
export async function getTx(txID: string): Promise<OrteliusAxiaTx> {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }

    let url = `v2/transactions/${txID}`;
    return await explorer_api.get<OrteliusAxiaTx>(url);
}
 ...
 * @param endTime
 */
async function getTransactionsAxia(
    addresses: string[],
    limit = 20,
    chainID: string,
    endTime?: string
): Promise<OrteliusAxiaTx[]> {
    if (!explorer_api) {
        throw NO_EXPLORER_API;
    }
    ...
    const res = await explorer_api.post<{ transactions: OrteliusAxiaTx[]; next?: string }>(rootUrl, req);
