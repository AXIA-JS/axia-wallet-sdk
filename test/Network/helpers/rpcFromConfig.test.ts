...
describe('RPC from Config helper methods', () => {
    it('rpc for C', () => {
        ...
        expect(rpc).toEqual(`https://api.axc.network:443/ext/bc/C/rpc`);
    });

    it('rpc for X', () => {
        let rpc = getRpcX(MainnetConfig);
        expect(rpc).toEqual(`https://api.axc.network:443/ext/bc/X`);
    });

    it('rpc for P', () => {
        let rpc = getRpcP(MainnetConfig);
        expect(rpc).toEqual(`https://api.axc.network:443/ext/bc/P`);
    });
});
